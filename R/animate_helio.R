...
for(i in 1:N) { #Number of frames
  if(i%%1 == 0) { #Sample every X frames in case there are too many of them
    ...
    png(paste("../heliocentric/frame",i,".png",sep=""),width=500,height=500)
    ...
    #Plot an empty graph
    plot(0,0,type="n",xlim=c(-1.2,1.2),ylim=c(-1.2,1.2),asp=1,main="Simulación: sistema solar (heliocéntrico).",xlab="x (ua)",ylab="y (ua)")
